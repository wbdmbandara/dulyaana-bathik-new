...
function ShopHeader() {
	...
	const [itemsPerPage, setItemsPerPage] = useState(
		new URLSearchParams(location.search).get("limit") || 12
	);
	...
	useEffect(() => {
		...
		const filters = {
			...
			fabrics: urlParams.get("fabrics")
				? urlParams.get("fabrics").split(",")
				: [],
		};
		...
		setSortOption(urlParams.get("sort") || "featured");
		setItemsPerPage(urlParams.get("limit") || 12);
	}, [location.search]);
								...
								<div className="filter-tags">
									...
									{activeFilters.fabrics &&
										activeFilters.fabrics.map((fabric) => (
											<span
												key={fabric}
												className="filter-tag text-capitalize"
											>
												{fabric}
											</span>
										))}
									<a className="clear-all-btn" href="/shop">
										Clear All
									</a>
								</div>
							</div>
